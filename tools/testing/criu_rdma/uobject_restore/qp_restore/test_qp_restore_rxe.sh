...
[ -x "$QUERY_BIN" ]   || { echo "build $QUERY_BIN first (make -C tools/testing/criu_rdma)"; exit 1; }
[ -x "$RESTORE_BIN" ] || { echo "build $RESTORE_BIN first (make -C tools/testing/criu_rdma)"; exit 1; }
