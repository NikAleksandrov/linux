...
>   (§S6b "Alternative C" backstop). **LANDED**.
...
>
> **Kernel agent: KS7.1 and KS7.2 are landed; KS7.3 is the
> only outstanding kernel ask.** Start at [§2](#2-kernel-agent-asks-at-a-glance);
...
The §S6b dmac-refresh helper that landed as
`mlx5_ib_restore_qp_refresh_av_dmac()` (`5166e228c223`) is correct
end-to-end but, on the v0 process-swap workload, hits `-ENOENT`
from `neigh_lookup` at restore time -- the operator's
`pin_static_neighbor_*` step in
`rdma_test_agent_vfmig_criu_swap_after_qp.yaml` is sequenced
**after** `criu restore`, so the destination netdev's ARP cache is
empty when `mlx5_ib_restore_qp` runs. Policy A then silently
no-ops, the QPC keeps its source-resolved (now-stale) `av.dmac`,
and the post-restore RC datapath continues to fail with
`IBV_WC_RETRY_EXC_ERR` (status 12, `vendor_err 0x81`).

Empirical lock from the kernel-agent diagnosis comment that
prompted this design:

```
Helper ran -- dmesg shows mlx5_ib_restore_qp_refresh_av_dmac:721
with the deferring-to-Policy-A message on both hosts.
Helper got err=-2 (-ENOENT) from neigh_lookup at restore time.
check_qp_av_dmac.sh later (post-CRIU) sees the pin IS there
(192.168.100.4 lladdr 02:00:f0:04:00:01 PERMANENT).
```

The dmac-refresh fix is sound, but the **timing** is wrong: the
helper needs to run after the operator has pinned ARP. CRIU's
restore is monolithic today, so there is no operator-visible
control point between LOAD_VHCA_STATE (step that brings up the
netdev) and RESTORE_QP (step that consumes the ARP cache).
...
1. **VF prerestore** -- a standalone CRIU-side binary
   ...
   that consumes the dump's plugin blob, drives the kernel to
   apply `LOAD_VHCA_STATE`, and **stops** before any uobject
   restore (no PD/CQ/MR/QP). The destination netdev surfaces;
   the operator can now run host-level network setup (in
   particular pinning the static ARP entry the §S6b dmac
   refresh helper needs).
...
2. **Process restore** -- a subsequent `criu restore` invocation.
   ...
   restore. Because the operator pinned ARP between phases,
   `mlx5_ib_restore_qp_refresh_av_dmac` finds the neighbor and
   actually applies the dmac refresh.
...
2. **An ioctl that re-runs the dmac refresh on demand** against
   an already-restored RC/UC QP, regardless of which CRIU phase
   the QP was restored in. Landed as
   `MLX5_VFMIG_IOC_REFRESH_AV_DMAC` (ioctl `0x12`,
   `b70b6624084a` + CLI wrapper `1bbe576bc7c5`). Serves as the
   §S6b "Alternative C" backstop so the datapath can be verified
   end-to-end ahead of the CRIU-side prerestore-binary work, and
   as the operator's recovery path if ARP is pinned after CRIU
   restore has already finished. See §4.
...
Surfaces (1) and (2) are LANDED. Surface (3) is the only
outstanding kernel work for v0. None of the three required
invasive changes to existing fast paths; all are additive.

## Status

Pre-implementation on the CRIU side. KS7.1 and KS7.2 are
landed; KS7.3 is the outstanding kernel-side ask (§2 / §3.5).
This doc is the contract for the CRIU-side work and the KS7.3
kernel work that follow.
...
unchanged from current main; the §S6b helper still runs and
still hits Policy A, but no test that was passing today
regresses. The plugin logs whether the VF was prerestored or
restored in-line so post-hoc analysis can tell the two paths
apart (see §6.4).
...
Two kernel-side surfaces are already in place (KS7.1, KS7.2).
One new small surface is required for safe identity matching of
prerestored VFs back to their CRIU dump images (KS7.3).
...
| KS7.2 | **`MLX5_VFMIG_IOC_REFRESH_AV_DMAC` ioctl** (§S6b "Alternative C" backstop). Landed in `b70b6624084a` with CLI wrapper in `1bbe576bc7c5`. Re-runs the `mlx5_ib_restore_qp_refresh_av_dmac()` neigh_lookup + RTS2RTS_QP refresh on an already-restored RC/UC QP, with rich pre/post bracketing (pre/post QUERY_QP, lookup_status, dmac_changed). Lets the operator pin ARP after CRIU restore, then refresh per migrated QP without redoing the whole restore. Unblocks end-to-end datapath validation independent of CRIU-side prerestore-binary work. | §4 | LANDED as ioctl 0x12 | -- |
...
## 4. KS7.2 -- `MLX5_VFMIG_IOC_REFRESH_AV_DMAC` (LANDED)
...
* `09d7fd51dc99` -- design notes in `qp_av_dmac_swap.md` §11

This section captures the as-built shape. The CRIU plugin and
the test harness consume the ioctl exactly as shipped; no
further kernel work is gated on this section.
...
The fallback path itself is unchanged from current main. The
§S6b helper still runs at QP restore time and still hits Policy
A if ARP isn't pinned yet. No regression for any existing test
that was passing before this work.
...
### 9.1 KS7.2-only path (immediate verification, no CRIU-side change)

Lets the kernel agent verify the §S6b helper end-to-end before
the CRIU-side prerestore-binary work lands.

```
criu_restore_host1 / host2: runs criu restore (monolithic flow,
                            today's path; helper hits Policy A)
pin_static_neighbor_*:      pin ARP on each destination netdev
mlx5_vfmig_refresh_dmac:    invokes MLX5_VFMIG_IOC_REFRESH_AV_DMAC
                            for each migrated QP on each host
                            (CLI verb landed in 1bbe576bc7c5)
ping_pong_after_restore:    traffic should now succeed
```

Pass criterion: `ping_pong_after_restore` returns OK MATCH on
both directions; `local_ack_timeout_err` does not tick during
the post-refresh window; QPs stay in RTS (no RTS->ERR).

Fail criterion: any of the above breaks. The most likely
failure mode (if the §S6b helper has a logic bug) is
`av.dmac` being refreshed to the wrong value -- caught by
re-running `check_qp_av_dmac.sh` post-refresh, which should
report `VERDICT=DMAC_IS_PEER` rather than `DMAC_IS_LOCAL`.

This path is what the kernel agent should expect to be running
for end-to-end validation while KS7.1 / CRIU-side
prerestore-binary work is in flight.
...
restore_vf_host2:        runs mlx5_vfmig_restore_vf on host2
                         (binary scans /dev/mlx5_vfmig/* for matching
                          vf_uuid; QUERY_VF.restored goes 0 -> 1 on
                          the matched slot)
pin_static_neighbor_*:   pin ARP on each destination netdev
                         (existing step, just moved earlier)
...
criu_restore_host2:      runs criu restore on host2
                         (init() looks up VF by vf_uuid, sees
                          restored=1, skips LOAD; in-restore dmac
                          helper sees pinned ARP, refreshes successfully)
ping_pong_after_restore: traffic should now succeed
```

Pass criterion: same as §9.1, plus the CRIU restore log on each
host emits the "prerestore detected; skipping LOAD_VHCA_STATE"
line from §6.4.
...
ping_pong_after_restore:    expected to succeed for VM-LM workloads,
                            ...
                            process-swap (then needs §9.1 backstop)
```
...
* **`set_vf_uuid` (new)**: invokes `MLX5_VFMIG_IOC_SET_VF_UUID`
  ...
  `tools/testing/mlx5_vfmig/tools/mlx5_vfmig.c` alongside
...
* **`refresh_av_dmac` (new)**: invokes the
  ...
  `1bbe576bc7c5`. Takes `host`, `vf_id`, `qpn`. Asserts exit
  code 0 plus the bracketing parser verdict
  (`REFRESHED_OK` / `ALREADY_OK`).
...
```yaml
# Same set_vf_uuid_host*_src / _dst as above (the soft-fallback
# path also requires the orchestrator-stamped UUID; the only
# difference is that there's no restore_vf step before
# criu_restore).
- name: criu_restore_host1
  # ... existing definition ...
- name: criu_restore_host2
...
- name: pin_static_neighbor_host2
  # ... existing definition ...
- name: refresh_av_dmac_host1
  type: refresh_av_dmac
  host: host1
  vf_id: 0
  qpn: 255
- name: refresh_av_dmac_host2
  type: refresh_av_dmac
  host: host2
  vf_id: 0
  qpn: 255
...
3. ~~**`refresh_qp_av` location.**~~ **Resolved.** Landed as
   `MLX5_VFMIG_IOC_REFRESH_AV_DMAC` (ioctl `0x12`) on the
   existing `/dev/mlx5_vfmig/<pf_bdf>` cdev, in `b70b6624084a`
   with CLI wrapper in `1bbe576bc7c5`. The cdev placement
   parallels the existing `MLX5_VFMIG_IOC_QUERY_QP` /
   `MLX5_VFMIG_IOC_PROBE_*` diagnostic surfaces.
