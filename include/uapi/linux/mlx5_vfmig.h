...
/*
 * MLX5_VFMIG_IOC_REFRESH_AV_DMAC:
 *   *** EXPERIMENTAL DEV-BACKUP IOCTL -- NOT part of the M2/M3
 *       contract. Carries the same "remove without breaking any
 *       in-tree consumer" disclaimer as the other PROBE_* /
 *       QUERY_* debug ioctls; if a future revision retires the
 *       always-on mlx5_ib_restore_qp_refresh_av_dmac path or
 *       collapses this surface into modify-qp, this ioctl can be
 *       dropped wholesale. ***
 *
 *   Companion to the always-on mlx5_ib_restore_qp_refresh_av_dmac
 *   path that runs from inside RESTORE_QP. That always-on path
 *   resolves av.dmac via neigh_lookup against the destination
 *   netdev's ARP / NDISC table at restore time, and applies
 *   Policy A (log + skip) when the neighbor entry isn't yet
 *   resolved. For the v0 mlx5_sriov_vfmig CRIU-on-host swap
 *   harness the criu plugin invokes RESTORE_QP atomically with
 *   the rest of VF restore -- BEFORE the harness's
 *   pin_static_neighbor_* step has populated the destination
 *   neighbor table. The always-on path therefore hits Policy A
 *   on every restored RC/UC QP, and the QPC is left with the
 *   stale source-resolved dmac.
 *
 *   This ioctl is the manual escape hatch for that case: the
 *   harness can run pin_static_neighbor_*, then call this ioctl
 *   per restored QP, and the dmac will be re-resolved against
 *   the now-populated ARP table. The implementation mirrors the
 *   mlx5_ib helper but lives entirely in mlx5_core/vfmig.c so it
 *   can run host-privileged on the bound VF mdev's cmdif (no
 *   cross-module call into mlx5_ib needed).
 *
 *   Sequence:
 *
 *     1. QUERY_QP(qpn) -- pre-op snapshot. Extract qpc.path
 *        primary-AV fields: vhca_port_num, src_addr_index,
 *        rgid_rip (dgid), and the current rmac_47_32 || rmac_31_0
 *        (cur_dmac).
 *
 *     2. Resolve the destination netdev for the VF
 *        (mlx5_uplink_netdev_get on the bound VF mdev) and
 *        neigh_lookup against the IPv4 / IPv6 GID for the
 *        peer's MAC. Cache-only, no ARP/NS solicit. Maps a
 *        missing or non-NUD_VALID entry to -ENOENT / -EAGAIN
 *        and surfaces it via @lookup_status.
 *
 *     3. If resolved_dmac == cur_dmac the QPC is already correct:
 *        @dmac_changed = 0, @op_* and @post_* are zeroed, and the
 *        ioctl returns success without disturbing the QP. (This
 *        is the common case for true VM live-migration where the
 *        source-preserved dmac is still valid.)
 *
 *     4. Otherwise issue MODIFY_QP(opcode=RTS2RTS_QP,
 *        opt_param_mask=PRIMARY_ADDR_PATH) with a qpc blob
 *        carrying only the new path.rmac_*. opt_param_mask gates
 *        which subfields FW consumes, so all other QPC fields
 *        (PSN / dest_qpn / sgid_index / etc.) stay untouched.
 *        @op_status / @op_syndrome capture the FW result.
 *
 *     5. Bracketing post-op QUERY_QP -- caller can verify
 *        post_dmac == resolved_dmac on success.
 *
 *   The @op step uses uid=0 (host-privileged); LOAD_VHCA_STATE
 *   wipes the QPC's owning-uid (per pd_registration_wipe.md
 *   §S3b empirical), so a uid=0 MODIFY_QP against a restored
 *   QPC's destination cmdif is the same path the always-on
 *   mlx5_ib helper takes.
 *
 *   Errors: -EFAULT on copy_{from,to}_user; -EINVAL if @vf_id is
 *   out of range, @qpn exceeds 24 bits, or any reserved field
 *   is non-zero; -ENODEV if the VF is unbound or its mdev
 *   interface is down or the VF has no associated netdev; any
 *   negative kernel/FW err code on cmdif transport failure for
 *   the bracketing QUERY_QP calls. The op-under-test does NOT
 *   propagate its negative errno: a FW reject is recorded in
 *   @op_status / @op_syndrome and the call returns 0 so the
 *   caller can see the bracketing query result.
 */
struct mlx5_vfmig_refresh_av_dmac {
	...
	__u32 pre_query_status;		/* out: 0 = QPC found pre-op;
					 *      non-zero = QUERY_QP failed.
					 *      On pre-query failure the
					 *      lookup / op / post-op steps
					 *      are SKIPPED and the
					 *      remaining out fields are
					 *      zeroed.
					 */
	__u32 pre_query_syndrome;	/* out: FW syndrome if pre-query
					 *      rejected; 0 on accept.
					 */
	__u8  pre_qpc_state;		/* out: qpc.state pre-op
					 *      (0=RST 1=INIT 2=RTR 3=RTS
					 *      4=SQEr 5=SQD 6=ERR).
					 */
	__u8  pre_vhca_port_num;	/* out: qpc.path.vhca_port_num
					 *      pre-op (1-based).
					 */
	__u8  reserved_pre0[2];		/* out: zeroed */
	__u32 pre_sgid_index;		/* out: qpc.path.src_addr_index
					 *      pre-op.
					 */
	__u8  pre_dmac[6];		/* out: qpc.path.rmac_47_32 ||
					 *      rmac_31_0 pre-op.
					 */
	__u8  reserved_pre1[2];		/* out: zeroed */
	__u8  pre_dgid[16];		/* out: qpc.path.rgid_rip pre-op
					 *      (16-byte raw GID).
					 */

	/* neighbor lookup against the VF's destination netdev */
	__s32 lookup_status;		/* out: 0 on found + NUD_VALID
					 *      (lladdr in @resolved_dmac);
					 *     -ENOENT no neighbor entry;
					 *     -EAGAIN entry present but
					 *      not NUD_VALID;
					 *     -ENODEV couldn't get the
					 *      VF's netdev;
					 *      other negative on
					 *      unexpected internal error.
					 *      Mirrors mlx5_ib_lookup_l2_dmac.
					 */
	__u8  resolved_dmac[6];		/* out: peer MAC from neighbor
					 *      table; meaningful only when
					 *      lookup_status == 0.
					 */
	__u8  dmac_changed;		/* out: 0 = resolved_dmac ==
					 *          pre_dmac; MODIFY_QP NOT
					 *          issued (already correct).
					 *      1 = MODIFY_QP was issued
					 *          (check @op_* and @post_*).
					 */
	__u8  reserved_lookup;		/* out: zeroed */

	/* op MODIFY_QP(RTS2RTS_QP, opt_param_mask=PRIMARY_ADDR_PATH) --
	 * only meaningful when @dmac_changed == 1.
	 */
	__u32 op_status;		/* out: 0 on FW accept;
					 *      kernel errno on FW reject
					 *      (cast through int). 0 when
					 *      @dmac_changed == 0.
					 */
	__u32 op_syndrome;		/* out: FW syndrome from
					 *      MODIFY_QP output blob; 0 on
					 *      FW accept. 0 when
					 *      @dmac_changed == 0.
					 */

	/* post-op QUERY_QP -- only meaningful when @dmac_changed == 1 */
	__u32 post_query_status;	/* out: 0 if post-query found
					 *      QPC; non-zero if FW
					 *      rejected the post-query.
					 *      0 when @dmac_changed == 0.
					 */
	__u32 post_query_syndrome;	/* out: FW syndrome on post-
					 *      query reject; 0 otherwise.
					 */
	__u8  post_qpc_state;		/* out: qpc.state post-op
					 *      (should equal
					 *      @pre_qpc_state). 0 when
					 *      @dmac_changed == 0.
					 */
	__u8  reserved_post0;		/* out: zeroed */
	__u8  post_dmac[6];		/* out: qpc.path.rmac post-op;
					 *      should equal
					 *      @resolved_dmac on FW accept.
					 *      All zero when
					 *      @dmac_changed == 0.
					 */
