				       ...
				       struct net_bridge_port_group *sg)
{
	...
	if (!br_multicast_is_star_g(&star_mp->addr) ||
	    !star_mp->host_joined)
		return;
			...
			u8 rt_protocol)
{
	...
	switch (group->proto) {
	case htons(ETH_P_IP):
		sg_port_add = !ipv4_is_zeronet(group->src.ip4);
		break;
#if IS_ENABLED(CONFIG_IPV6)
	case htons(ETH_P_IPV6):
		sg_port_add = !ipv6_addr_any(&group->src.ip6);
		break;
#endif
	}
	if (sg_port_add) {
		if (rhashtable_lookup_insert_fast(&port->br->sg_port_tbl,
						  &p->rhnode,
						  br_sg_port_rht_params)) {
			kfree(p);
			return NULL;
