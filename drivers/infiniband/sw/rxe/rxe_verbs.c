...
/*
 * CRIU-restore variant of rxe_create_qp (S6a A2). The generic
 ...
 * the v0 set {RC,UD}, validated qp_state against {RESET,INIT,RTR,RTS},
 * rejected any SRQ reference (no RESTORE_SRQ at v0), and pre-stamped
 * the ib_qp shell (device/pd/qp_type/send_cq/recv_cq/event handlers,
			  ...
			  struct ib_udata *udata)
{
	 ...
	 * shared cdev VMA that CRIU does not snapshot, so unreaped CQEs and the
	 * cursors are shipped through QUERY_CQ/RESTORE_CQ instead (mirrors the
	 * QP SQ/RQ image path). The cursors are validated and seeded
	 * unconditionally (cheap, and correct for a wrapped-but-empty ring);
	 * the ring image, when present (cqe_image_bytes > 0 and an UHW_IN tail),
	 * is blitted first. The user CQ ring is QUEUE_TYPE_TO_CLIENT so the seed
	 * goes through rxe_cq_seed_ring (kernel-owned producer), NOT
	 * rxe_qp_seed_ring -- see the helper for why reuse would clobber slot 0.
	 */
				    ...
				    struct ib_udata *udata)
{
	...
	if (err) {
		 ...
		 */
		rxe_dbg_pd(pd,
			   "rxe_restore_mr: pool install at index 0x%x failed, err = %d\n",
			   index_hint, err);
		goto err_free;
	}
	...
	if (err) {
		rxe_dbg_mr(mr, "rxe_restore_mr: rxe_mr_init_user failed, err = %d\n",
			   err);
		goto err_cleanup;
	}
	 ...
	 */
	mr->lkey = mr->ibmr.lkey = lkey_hint;
	mr->rkey = mr->ibmr.rkey = rkey_hint;
...
err_cleanup:
	cleanup_err = rxe_cleanup(mr);
	if (cleanup_err)
		rxe_err_mr(mr, "rxe_restore_mr: cleanup failed, err = %d\n",
			   cleanup_err);
err_free:
	kfree(mr);
	return ERR_PTR(err);
