	...
	struct uverbs_attr_bundle *attrs)
{
	...
	obj->uevent.event_file = ib_uverbs_get_async_event(
		attrs, UVERBS_ATTR_RESTORE_CQ_EVENT_FD);
	...
	struct uverbs_attr_bundle *attrs)
{
	...
	obj->uevent.event_file = ib_uverbs_get_async_event(
		attrs, UVERBS_ATTR_RESTORE_QP_EVENT_FD);
	uobj->user_handle = user_handle;
	...
	/*
	 * Post-driver: stamp the cap echo back. Drivers may have
	 * rounded up max_*_wr to the actual installed WQ depth.
	 */
