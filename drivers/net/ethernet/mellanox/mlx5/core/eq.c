	      ...
	      struct mlx5_eq_param *param)
{
	...
	/*
	 * EQ frag buffers go in their own vfmig IOVA slot so addition
	 * or removal of an EQ on the destination doesn't shift the
	 * IOVAs of WQ frag buffers (FRAG_BUF) or doorbell pgdir pages
	 * (DB_PAGE). On an untracked VF the slot argument is ignored
	 * and the legacy dma_alloc_coherent path runs.
	 */
